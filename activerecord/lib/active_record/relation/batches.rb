      ...
      def batch_on_unloaded_relation(relation:, start:, finish:, load:, order:, use_ranges:, remaining:, batch_limit:)
        ...
        loop do
          if load
            ...
            # We retry by getting the whole list of remaining ids so that we have the exact size and last id.
            unless ids_last
              ids = batch_relation.ids
              ids_size = ids.size
              ids_last = ids.last
            end
            # Finally, build the yielded relation if at least one id found
            if ids_last
