    ...
    module PostgreSQL
      module SchemaStatements
        ...
        def table_options(table_name) # :nodoc:
          ...
          if supports_table_inheritance? && parent_table_names = inherited_table_names(table_name).presence
            options[:options] = "INHERITS (#{parent_table_names.join(", ")})"
          end
          if !options[:options] && supports_native_partitioning? && partition_definition = table_partition_definition(table_name)
